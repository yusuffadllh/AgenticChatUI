 ...
 */
export async function runOpencode({ prompt, cwd, settings, signal, onOutput, idleTimeoutMs = 300000, maxTimeoutMs = 1200000 }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // OpenCode buffers its logs when stdout/stderr aren't a TTY (as under Node's
    // child_process), so nothing reaches the UI until it exits — which makes the
    // idle watchdog wrongly kill it. Wrap with `stdbuf -oL -eL` to force
    // line-buffered output so logs stream in real time; fall back to a direct
    // spawn if stdbuf is unavailable.
    try {
      child = spawn('stdbuf', ['-oL', '-eL', bin, ...args], { cwd, env, shell: false });
    } catch {
      child = spawn(bin, args, { cwd, env, shell: false });
      wireChild(false);
      resetIdle();
